...
{
    const auto w = INDICATOR_MARGIN.left() + SWITCH_HEIGHT + INDICATOR_MARGIN.right();
    return ltr(this) ? QRect(0, 0, w, SWITCH_HEIGHT) : QRect(width() - w, 0, w, SWITCH_HEIGHT);
}
...
{
    auto h = SWITCH_HEIGHT;
    auto w = INDICATOR_MARGIN.left() + SWITCH_HEIGHT + INDICATOR_MARGIN.right() + fontMetrics().horizontalAdvance(text());

    return QSize(w, h);
}

void Switch::changeSize(float factor)
{
    CORNER_RADIUS = factor * 5.4;
    THUMB_RADIUS = factor * 10;
    SHADOW_ELEVATION = factor * 1.4;
    SWITCH_HEIGHT = factor * 26.0;
    MARGINS = factor * 20./3;
    INDICATOR_MARGIN = QMargins(MARGINS, MARGINS, MARGINS, MARGINS);
}
...
void Switch::paintEvent(QPaintEvent*)
{
    ...
    auto trackMargin = INDICATOR_MARGIN;
    trackMargin.setTop(MARGINS);
    trackMargin.setBottom(MARGINS);
    ...
    if (isEnabled())
    {
        ...
        if (!shadowPixmap.isNull()) p.drawPixmap(thumbRect.center() - QPointF(THUMB_RADIUS, THUMB_RADIUS - 1.0), shadowPixmap);
    ...
    else
    {
        ...
        // draw thumb
        p.setOpacity(1.0);
        if (!isChecked()) trackRect.setX(trackRect.x() - trackMargin.left() - trackMargin.right() - 2);
...
void Switch::toggle(Qt::CheckState state)
{
    if (state == Qt::Checked)
    {
        const QVariant posEnd = (INDICATOR_MARGIN.left() + INDICATOR_MARGIN.right() + 2) * 2;
